...
#define R_ARGS vec2( gl_FragCoord.x/c_Size.x, gl_FragCoord.y/c_Size.y ), rng
...
vec4 spheres[8] = vec4[](
                         ...
                         vec4(0.0, -1e5 - 1.0, 0.0, 1e5),
                         vec4(0.0,  1e5 + 1.0, 0.0, 1e5),
                         
                         vec4(-1e5 - 1.0, 0.0, 0.0, 1e5),
                         vec4( 1e5 + 1.0, 0.0, 0.0, 1e5),
                         
                         vec4(0.0, 0.0, 1.0 + 1e5, 1e5)
                         );
...
void main(void)
{
	...
	const int nIter = 10;
	const float aolength = 0.75;
	for(int ie=0; ie<nIter; ie++)
	{//	shadow ray cast
		float4 f1 = from + hit.m_f * (to - from);
		f1 += hit.m_hitNormal * RAYCAST_EPSILON;

		float4 t1 = f1 + aolength*getRandomHemiSphericalCosWeighted( hit.m_hitNormal, draw(R_ARGS), draw(R_ARGS) );
